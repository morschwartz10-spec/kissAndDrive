...
function renderRegisterView(container) {
    ...
    document.getElementById('registerForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (existingUser) {
                // Phone already exists — block duplicate registration
                showToast(`⚠️ מספר הטלפון ${phone} כבר רשום במערכת על שם "${existingUser.name}". לא ניתן להירשם פעמיים. עבור ל"שלי" לניהול משמרות.`);
                btn.disabled = false;
...
async function handleModalSubmit() {
    ...
    if (volunteer) {
        // Check if volunteer is already assigned to this date
        const existingShift = appData.shifts[dateStr] || [];
        if (existingShift.find(s => s.volunteerId === volunteer.id)) {
            showToast(`⚠️ ${volunteer.name}, את/ה כבר משובצ/ת למשמרת ביום זה!`);
            window.pendingShiftDate = null;
            return;
        }
